...
@router.post("/shorten")
async def shorten(req: dict, service: Annotated[URLService, Depends(get_service)]):
    code = await service.create(req["url"])
    return {"shortUrl": f"{settings.BASE_URL}/{code}"}

@router.get("/{code}")
async def redirect(code: str, service: Annotated[URLService, Depends(get_service)]):
